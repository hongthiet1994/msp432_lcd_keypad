...
{
  int8_t h;
  GPIO_setOutputLowOnPin(Port_lcd,Cs);
  if(cd)
  {
    GPIO_setOutputHighOnPin(Port_lcd, A0);
  }
  else
  {
    GPIO_setOutputLowOnPin(Port_lcd, A0);
  }
  for(h=0;h<8;h++)
  {
    if(byte&0x80) 
    {
      GPIO_setOutputHighOnPin(Port_lcd, SDA);
    }
    else
    {
      GPIO_setOutputLowOnPin(Port_lcd, SDA);
    }
    
    GPIO_setOutputLowOnPin(Port_lcd, SCLK);
    GPIO_setOutputHighOnPin(Port_lcd, SCLK);
    
    byte<<=1;
  }
  GPIO_toggleOutputOnPin(Port_lcd,A0);
  GPIO_setOutputHighOnPin(Port_lcd, Cs);
